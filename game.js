...
console.log('go get new word', sonda);
...
var nova_palavra = selectedWords[num]; //.toLowerCase();
...
nova_palavra = selectedWords[num+1]
}
...
const vetores = await getModel(palavra_sonda.toLocaleLowerCase(), palavra_respondida.toLocaleLowerCase());
...
if (!vetores.vec_2) {
    ...
    time_in = new Date();
    setTimeout(() => {
        palavra_sonda = getNewWord(palavra_sonda);
